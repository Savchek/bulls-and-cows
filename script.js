...
let pl = document.getElementById('play'),
	tr = document.getElementById('turns'),
	sr = document.getElementById('surrender')
...
start = () => {
	...
	document.getElementById('right').scrollTop = -999
	planning = true
}
...
blink = (el, color, regularColor, time) => {
	el.style.background = color
	setTimeout(() => el.style.background = regularColor || '#C5FFFF', time || 200)
}
...
compare = () => {
	...
	if(bulls == 4){
		wrt('Ви виграли🎉 Число: ' + sNum.join('') + ', Хід #' + (--turn))
		...
		if (botRes.bulls == 4){
			...
			wrt('🤖Спроба суперника: У числі ' + botRes.num.join('') + bguessed)
			wrt('🤔️Ваша спроба: У числі ' + data.join('') + pguessed)
		}
		data=[]
	}
}
